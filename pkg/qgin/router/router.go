...
import (
	...
	"github.com/qdriven/qfluent-go/internal/logging"
	ginRequestId "github.com/qdriven/qfluent-go/pkg/qgin/middleware/request"
)

var logger = logging.ZoneLogger("gin/router")

// NewRouter creates a new router (a gin.New() router)
// with gin.Recovery() middleware, the logging.Logger4Gin middleware,
...
func NewRouter(options ...RouterOption) *gin.Engine {
	router := gin.New()
	router.Use(gin.Recovery(), logging.Logger4Gin, ginRequestId.RequestID())
